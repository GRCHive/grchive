...
ADD COLUMN approve_time TIMESTAMPTZ;

ALTER TABLE document_requests
ADD COLUMN approve_user_id BIGINT REFERENCES users(id) ON DELETE CASCADE;
...
$$
    DECLARE
        ...
        APPROVED constant INT := 5;
    BEGIN
        IF r.approve_time IS NOT NULL AND r.approve_user_id IS NOT NULL THEN
            RETURN APPROVED;
        ELSIF r.completion_time IS NULL THEN
            IF r.due_date IS NOT NULL THEN
                IF NOW() <= r.due_date THEN
                    IF r.progress_time IS NOT NULL THEN
                        RETURN PROGRESS;
                    ELSE
                        RETURN OPEN;
                    END IF;
