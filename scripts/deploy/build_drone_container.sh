...
if [[ -z $MINIKUBE ]]; then
    FULL_IMAGE_URL=registry.gitlab.com/grchive/grchive/drone:`git rev-parse HEAD`
    docker tag bazel/devops/docker/drone:drone $FULL_IMAGE_URL
    docker push $FULL_IMAGE_URL

    FULL_IMAGE_URL=registry.gitlab.com/grchive/grchive/drone_runner_k8s:`git rev-parse HEAD`
    docker tag bazel/devops/docker/drone_runner:drone-runner-k8s $FULL_IMAGE_URL
    docker push $FULL_IMAGE_URL

    FULL_IMAGE_URL=registry.gitlab.com/grchive/grchive/drone_runner_worker_image:latest
